...
def listar_empresas(request):
    if request.method == "GET":
        ...
        pesquisa = request.POST.get('empresa')
        empresas = Empresas.objects.filter(user = request.user, nome__icontains=pesquisa)
        return render(request, 'listar_empresas.html', {'empresas': empresas})
...
def add_doc(request, id):
    ...
    # if empresa.user != request.user:
    #     messages.add_message(request, constants.ERROR, "Empresa não é sua")
    #     return redirect(f'/empresarios/listar_empresas')
